...
If multiple filenames share a numeric prefix, the current generator deterministically keeps the first sorted file. Future authoring must reject that ambiguity before generation; it must not depend on first-file-wins behavior.
...
- `admin-ui/src/thumbs.manifest.js` is generated and can be regenerated by Admin build/dev commands.
